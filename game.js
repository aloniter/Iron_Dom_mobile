class IronDOMGame {
            ...
            for (let j = this.interceptors.length - 1; j >= 0; j--) {
                ...
                if (distance < 40) {
                    ...
                    if (this.intercepts >= this.targetIntercepts) {
                        this.startTrumpAnimation(() => this.victory());
                    }
                    break;
                }
            }
    ...
    updateTrumpAnimation(deltaTime) {
        ...
        // Start text fade in when Trump reaches 3/4 of the screen (much earlier)
        if (this.trumpAnimation.x <= this.canvasWidth * 0.75 && !this.trumpAnimation.textFadeIn) {
            this.trumpAnimation.textFadeIn = true;
        }
        
        // Fade in text slower
        if (this.trumpAnimation.textFadeIn && this.trumpAnimation.textAlpha < 1) {
            this.trumpAnimation.textAlpha += deltaTime / 800; // Fade in over 0.8 seconds (slower)
        }
        
        // End animation when Trump exits screen and enough time has passed (longer duration)
        if (this.trumpAnimation.x <= -this.trumpAnimation.width && this.trumpAnimation.duration > 5000) {
